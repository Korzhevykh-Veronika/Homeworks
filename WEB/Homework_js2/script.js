...
function createNewElements(arr) {
    for (key in arr) {
        ...
        if (arr[key].price.newUan !== undefined) {
            let newPrice = document.createElement('div');
            newPrice.className = "phone-new-price";
            newPrice.innerHTML = arr[key].price.newUan;
            div.append(newPrice);
            let oldPrice = document.createElement('div');
            oldPrice.className = "phone-old-price";
            oldPrice.innerHTML = arr[key].price.oldUan;
            div.append(oldPrice);
        } else {
            let price = document.createElement('div');
            price.className = "phone-price";
            price.innerHTML = arr[key].price;
            div.append(price);
        }

        let revievs = document.createElement('div');
        revievs.className = "phone-revievs";
        revievs.innerHTML = arr[key].ratingRevievs;
        div.append(revievs);

    }
}

function newPhonePrice(arr) {
    for (key in arr) {
        if (arr[key].price.newUan !== undefined)
            arr[key].price = parseInt(arr[key].price.newUan.slice(0, -4).replace(/\s+/g, ''), 10);
        else arr[key].price = parseInt(arr[key].price.slice(0, -4).replace(/\s+/g, ''), 10);
    }
    sortArray(arr);
}
...
createNewElements(phones);
newPhonePrice(phones);
